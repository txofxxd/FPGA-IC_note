...
| 阶段         | 名称                    | 输入           | 输出             | 做的事情（核心）                      | 你能控制什么                     | 典型影响         |
| ------------ | ----------------------- | -------------- | ---------------- | ------------------------------------- | -------------------------------- | ---------------- |
| RTL          | Register Transfer Level | Verilog / VHDL | RTL网表          | 只做语法级建模，不优化                | 编码方式（FSM写法/结构）         | 代码“长什么样”   |
| synth_design | 综合                    | RTL            | 门级网表（初级） | 转成 LUT + FF 结构                    | FSM encoding / attribute         | 是否识别FSM      |
| opt_design   | 优化（Optimization）    | 网表           | 优化后网表       | 去冗余、合并逻辑、寄存器优化、FSM优化 | keep / dont_touch / fsm_encoding | 逻辑是否被改写   |
| place_design | 布局                    | 优化网表       | 物理位置分配     | 把 LUT/FF 放到 FPGA 具体资源上        | LOC约束                          | 延迟开始“真实化” |
| route_design | 布线                    | 布局结果       | 完整实现电路     | 连线资源分配（routing）               | timing constraints               | 最终时序         |
